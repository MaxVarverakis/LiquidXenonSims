...
SensitiveTarget::SensitiveTarget(G4String name) : G4VSensitiveDetector(name)
{
...
void SensitiveTarget::Initialize(G4HCofThisEvent* hce)
{
    ...
    // fill target hits with zero energy deposition
    fHitsCollection -> insert(new TargetHit());
}

G4bool SensitiveTarget::ProcessHits(G4Step *step, G4TouchableHistory *R0history)
{
